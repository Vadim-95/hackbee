...
def get_open_channels(dev_id, channel, verbose=False, file_path=None, delay=2):
    '''
    ...
    if verbose:
        networkdata,txcount, rxcount = os.system("sudo zbstumbler -i {0} -c {1} -s {2} -w {3} -v".format(dev_id, channel, delay, file_path))
    else:
        networkdata,txcount, rxcount = os.system("sudo zbstumbler -i {0} -c {1} -s {2} -w {3}".format(dev_id, channel, delay, file_path))